"""
Main training pipeline for knowledge distillation with Swin Transformer Tiny teacher and lightweight CNN student.
...
from trainers.teacher_trainer import TeacherTrainer; from trainers.student_kl_contrastive_l2_bce import StudentTrainer_KL_Contrastive_L2_BCE as StudentTrainer
...
def train_single_seed(
    ...
    paths = create_output_dirs(output_base, 'swin-tiny_kl_contrastive_l2_bce', seed)
    ...
    # =========================================================================
    print("\n" + "-"*80)
    print("TEACHER TRAINING (Multimodal Swin Transformer Tiny)")
    print("-"*80)
    
    teacher_model = MultimodalSwinTiny(num_classes=2, freeze_until_stage=1)
    ...
    plot_training_curves(
        ...
        title=f'Swin-Tiny Teacher Training (Seed {seed})'
    )
    ...
    # Evaluate Teacher
    print("\nEvaluating Swin Transformer Tiny Teacher Model...")
    teacher_model.load_state_dict(torch.load(teacher_save_path))
    ...
    plot_embeddings(
        ...
        method='tsne',
        title=f'Swin-Tiny Teacher TSNE (Seed {seed})'
    )
    ...
    plot_embeddings(
        ...
        method='umap',
        title=f'Swin-Tiny Teacher UMAP (Seed {seed})'
    )
...
def main():
    parser = argparse.ArgumentParser(
        description='Knowledge Distillation Pipeline with Swin Transformer Tiny Teacher'
    )
    ...
    print("="*80)
    print("KNOWLEDGE DISTILLATION PIPELINE - SWIN TRANSFORMER TINY TEACHER")
    print("="*80)
    ...
    if all_results:
        ...
        aggregated = {
            ...
            'metadata': {
                ...
                'teacher_architecture': 'swin-tiny_kl_contrastive_l2_bce'
            }
        }
        
        # Save aggregated results
        aggregated_path = os.path.join(args.output, 'metrics', 'swin-tiny_kl_contrastive_l2_bce', 'aggregated_results.json')
