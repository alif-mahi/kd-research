"""
Student trainer with KL + L2 + BCE loss (New variant for ablation study).
Loss: alpha * KL_loss + beta * L2_loss + gamma * BCE_loss

L2 loss performs feature matching between student and teacher representations.
"""
...
class StudentTrainer_KL_L2_BCE:
    """Student trainer using KL divergence + L2 feature matching + BCE loss."""
    ...
    def __init__(
        ...
        beta: float = 0.2,   # L2 weight
        gamma: float = 0.3,  # BCE weight
        lr: float = 1e-3,
        ...
        Args:
            ...
            beta: Weight for L2 feature matching loss
            gamma: Weight for BCE loss
            lr: Learning rate
    ...
    def train_epoch(self, teacher_loader: DataLoader) -> Dict[str, float]:
        """
        Train for one epoch using triple loss (KL + L2 + BCE).
        ...
        pbar = tqdm(
            ...
            desc='Training Student (KL+L2+BCE)',
        ...
        for (rgb_student, labels_student), (rgb_teacher, ir_teacher, labels_teacher) in pbar:
            ...
            loss = self.alpha * kd_loss + self.beta * l2_loss + self.gamma * hard_loss
    ...
    def train(
        ...
        """
        print(f"Training student model with KL+L2+BCE for {num_epochs} epochs...")
        print(f"Loss weights: alpha={self.alpha} (KL), beta={self.beta} (L2), gamma={self.gamma} (BCE)")
